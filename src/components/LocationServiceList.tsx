        ...
        {AllServices.ServiceArray.map((service, index) => {
          ...
          if (fullService) {
            return (
              <Link
                key={index}
                href={`/services/${services.find(s => s.name === service.title)?.slug}`} // Direct slug from services
                className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 border border-gray-200 hover:border-green-200 relative h-96"
              >
                <div
                ...
                >
                  ...
                  <div className="left-0 top-1/3  relative h-full flex flex-col p-6 text-white">
                    ...
                    <button className="bg-[var(--primary-color)] hover:bg-green-500 text-white font-medium px-4 py-2 rounded-lg cursor-pointer transition-all transition-transform hover:scale-105 duration-300">{service.buttonText}</button>
                  </div>
                </div>
              </Link>
            );
